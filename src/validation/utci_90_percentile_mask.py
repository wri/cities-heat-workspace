...
def validate_utci_from_config(city, local_utci_paths, global_utci_paths, mask_path, all_absolute_errors_by_time):
    ...
    for time, local_path, global_path in zip(base_time_steps, local_utci_paths, global_utci_paths):
        ...
        try:
            with rasterio.open(local_path) as src_local, rasterio.open(global_path) as src_global, rasterio.open(mask_path) as src_mask:
                ...
                aligned = (
                    src_local.crs == src_global.crs == src_mask.crs  and
                    src_local.transform == src_global.transform == src_mask.transform  and
                    src_local.shape == src_global.shape == src_mask.shape  and
                    src_local.bounds == src_global.bounds == src_mask.bounds 
                )
                
                if not aligned:
                    print(f"🟠 {time}: Raster mismatch. Cropping to overlap and trimming boundary.")
                    win_local, win_global, win_mask = get_overlap_window([src_local, src_global, src_mask])
        ...
        mask_valid = (mask_data_cropped == 1) # binary masks for non_building
        valid_mask = (~np.isnan(local_data)) & (~np.isnan(global_data))
        combined_mask = valid_mask & mask_valid # valied pixels and choosing only the non_building pixels
...
def main():
    ...
    for city_name, city_config in all_configs.items():
        ...
        mask_path = city_config['mask_paths']['land_use_mask_path']

        validate_utci_from_config(city_name, local_utci_paths, global_utci_paths, mask_path, all_absolute_errors_by_time)
    ...
    summary_df.to_csv(output_dir / "utci_non_building_percentiles.csv", index=False)
