...
def plot_utci_pixel_scatter(local_utci_paths, global_utci_paths, shade_local_paths, shade_global_paths, output_dir, city_name):
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    time_steps = [Path(p).stem.split('_')[-1] for p in local_utci_paths]
    
    # call both local and global shade masks
    shade_strategies = {
        'LocalShade': {'paths': shade_local_paths, 'label': 'Local Shade Mask'},
        'GlobalShade': {'paths': shade_global_paths, 'label': 'Global Shade Mask'},
    }
    
    # First pass: collect all data to determine consistent axis ranges for each timestamp
    all_data = {}
    
    for strategy_name, strategy_info in shade_strategies.items():
        all_data[strategy_name] = {}
        shade_paths = strategy_info['paths']
        
        for t, local_path, global_path, shade_path in zip(time_steps, local_utci_paths, global_utci_paths, shade_paths):
            print(f"Processing {strategy_name} - {t}: {local_path} vs {global_path}")
            ...
            try:
                with rasterio.open(local_path) as src_local, rasterio.open(global_path) as src_global, rasterio.open(shade_path) as src_shade:
                    ...
                    if not aligned:
                        print(f"🟠 {strategy_name} - {t}: Raster mismatch. Cropping to overlap and trimming boundary.")
                        ...
                        print(f"🟢 {strategy_name} - {t}: Rasters aligned. Trimming boundary.")
                ...
                print(f"❌ Error reading files for {strategy_name} - {t}: {e}")
                continue
            ...
            for mask_name, mask_func in MASK_LABELS.items():
                ...
                all_data[strategy_name][t][mask_name] = {
                    'local': y_true_sample,
                    'global': y_pred_sample
                }
    
    # Determine consistent axis ranges for each timestamp (across all strategies and mask types)
    axis_ranges = {}
    for t in time_steps:
        all_local = []
        all_global = []
        for strategy_name in shade_strategies.keys():
            if t in all_data[strategy_name]:
                for mask_name, data in all_data[strategy_name][t].items():
    ...
    # Create 3x1 subplot for each mask type and shade strategy combination
    for strategy_name, strategy_info in shade_strategies.items():
        for mask_name in MASK_LABELS.keys():
            timestamps_with_data = [t for t in time_steps 
                                  if t in all_data[strategy_name] and mask_name in all_data[strategy_name][t]]
            ...
            for i, t in enumerate(timestamps_with_data):
                data = all_data[strategy_name][t][mask_name]
            ...
            # Update title to include shade strategy
            fig.suptitle(f'{city_name}: UTCI {mask_name} - {strategy_info["label"]}', fontsize=16)
            plt.tight_layout()
            
            # Save the combined plot with strategy name
            output_path = output_dir / f"utci_pixel_scatter_{mask_name}_{strategy_name}_3x1.png"
...
def main():
    ...
    # generate all plots
    plot_utci_pixel_scatter(local_utci_paths, global_utci_paths, shade_local_paths, shade_global_paths, output_dir, city_name)
