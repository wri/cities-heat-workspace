...
mrt_dir = "/Users/hyejijoh/Desktop/rdj/"
...
met_file = "/Users/hyejijoh/Desktop/rdj/metforcing.txt"

## folder to save UTCI output files
out_dir = "/Users/hyejijoh/Desktop/rdj/"
...
def check_inputs(at, wind, vpd, mrt_arr, utc_offset):
	...
	if not -50 <= at <= 50:
		...
		raise SystemExit('Vapor pressure deficit must be between 0 and 50 hPa! vpd is', vpd)
...
def vpd_from_rh(at, rh):
	# calculates air vpd in hPa (hecto pascals) for utci calculation
	svp = 610.78*(2.72**(at/(at+237.3)*17.2694))
...
for i, met in df.iterrows():
	...
	# open mrt array
	pattern = mrt_dir+'Tmrt_'+year+'_'+day+'_'+hourmin+'*.tif'
	print("Looking for MRT file:", pattern)
	mrt_file = glob.glob(pattern)[0]
