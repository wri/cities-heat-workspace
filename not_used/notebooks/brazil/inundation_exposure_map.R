...
census_tract <- st_read("https://cities-socio-economic-vulnerability.s3.eu-west-3.amazonaws.com/case-studies/campinas/maps/inundation_exposed75_tract.geojson")
...
pal_hazard <- colorNumeric("RdYlBu", 
                           census_tract$Inundation_indice_sum_mean, #change
                           na.color = "grey",
                           reverse = TRUE)
...
labels_hazard <- sprintf("<strong>%s:</strong> %s ", #change
                         "Average Inundation Risk Index", round(census_tract$Inundation_indice_sum_mean,3)
) %>% 
  lapply(htmltools::HTML)
...
addPolygons(data = census_tract,
  ...
  # hazard ----
addPolygons(data = census_tract,
            group = "Average Inundation Risk Index", #change
            fillColor = ~pal_hazard(Inundation_indice_sum_mean), #change
  ...
  addLegend(pal = pal_hazard, #change
            values = census_tract$Inundation_indice_sum_mean, #change
            opacity = 0.9,
            group = "Average Inundation Risk Index", #change
            position = "topright",
            title = "Average Inundation Risk Index", #change
            labFormat = labelFormat(suffix = "")) %>%
...
addLayersControl(
  ...
  overlayGroups = c("Total Population", "Average Inundation Risk Index", "Female Population", "Male Population",
  ...
  hideGroup(c("Average Inundation Risk Index", "Female Population", "Male Population",
              "White Population", "Black Population", "Yellow Population", "Brown Population", "Indigenous Population",
              "Population with Age Lesser than 14", "Population with Age Between 15 and 19",
              "Population with Age Between 20 and 59", "Population with Age Above 60",
              "Average Income"))
