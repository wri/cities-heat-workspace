...
census_tract <- st_read("https://cities-socio-economic-vulnerability.s3.eu-west-3.amazonaws.com/case-studies/campinas/maps/landslide_exposed75_tract.geojson")
...
pal_hazard <- colorNumeric("RdYlBu", 
                           census_tract$Landslide_indice_sum_mean, #change
                           na.color = "grey",
                           reverse = TRUE)
...
labels_hazard <- sprintf("<strong>%s:</strong> %s ", #change
                         "Average Landslide Risk Index", round(census_tract$Landslide_indice_sum_mean,3)
) %>% 
  lapply(htmltools::HTML)
...
addPolygons(data = census_tract,
  ...
  # hazard ----
addPolygons(data = census_tract,
            group = "Average Landslide Risk Index", #change
            fillColor = ~pal_hazard(Landslide_indice_sum_mean), #change
  ...
  addLegend(pal = pal_hazard, #change
            values = census_tract$Landslide_indice_sum_mean, #change
            opacity = 0.9,
            group = "Average Landslide Risk Index", #change
            position = "topright",
            title = "Average Landslide Risk Index", #change
            labFormat = labelFormat(suffix = "")) %>%
...
addLayersControl(
  ...
  overlayGroups = c("Total Population", "Average Landslide Risk Index", "Female Population", "Male Population",
  ...
  hideGroup(c("Average Landslide Risk Index", "Female Population", "Male Population",
              "White Population", "Black Population", "Yellow Population", "Brown Population", "Indigenous Population",
              "Population with Age Lesser than 14", "Population with Age Between 15 and 19",
              "Population with Age Between 20 and 59", "Population with Age Above 60",
              "Average Income"))
