...
###################################
# Spatial distribution maps - Census Tract Variables
###################################

# get hazard data at the census tract level ----
census_tract <- st_read("https://cities-socio-economic-vulnerability.s3.eu-west-3.amazonaws.com/case-studies/campinas/maps/census_tract_keep.geojson")
census_tract <- st_transform(census_tract, crs = 4326)
  ...
  labels_p_indig <- sprintf("<strong>%s:</strong> %s <br/><strong>%s:</strong> %s <br/><strong>%s:</strong> %s", #change
                            "Total Indigenous Population", census_tract$p_indig, #change
                            "Share of Indigenous Population in Tract", census_tract$prop1_p_indig, #change
                            "Share in Total Indigenous Population of City", census_tract$prop2_p_indig #change
  ) %>% 
    lapply(htmltools::HTML) 
...
addLegend(pal = pal_p_tot, #change
  ...
  addLayersControl(
    ...
    overlayGroups = c("Total Population", "Female Population", "Male Population",
  ...
  hideGroup(c("Female Population", "Male Population",
              "White Population", "Black Population", "Yellow Population", "Brown Population", "Indigenous Population",
              "Population with Age Lesser than 14", "Population with Age Between 15 and 19",
              "Population with Age Between 20 and 59", "Population with Age Above 60",
              "Average Income"))
