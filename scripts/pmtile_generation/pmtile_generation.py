...
def convert_geojson_to_pmtiles(
    geojson_file_location: str, pmtiles_file_location: str
) -> Union[str, None]:
    """
...
if __name__ == "__main__":
    ...
    for layer_url in layer_urls:
        ...
        error_str = download_file_from_url(
            layer_url, f"./data/{str(geojson_file_name)}"
        )
        if error_str:
            print(error_str)
            continue
        error_str = convert_geojson_to_pmtiles(
            f"./data/{geojson_file_name}", f"./data/{pmtiles_file_name}"
        )
        if error_str:
            print(error_str)
            continue
        error_str = upload_file_to_s3_bucket(
            f"./data/{pmtiles_file_name}", "cities-indicators", s3_pmtiles_path_name
        )
        if error_str:
            print(error_str)
            continue
